...
interface Props {
  onFileSelected: (filePath: string, fileName: string, language: string) => void;
  modelName: string;
  onChangeModel: () => void;
}
...
export function DropZone({ onFileSelected, modelName, onChangeModel }: Props) {
  ...
  const handlePick = useCallback(async () => {
    ...
    if (file) {
      onFileSelected(file.path, file.name, language);
    }
  }, [language, onFileSelected]);
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-5 p-8">
      ...
      {/* Drop Zone - themed with logo colors */}
      <div
        onClick={handlePick}
      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="text-xs mt-1" style={{ color: 'rgba(148, 163, 184, 0.8)' }}>
            mp3, wav, m4a, flac, ogg, aac, mp4, mov, webm, mkv, avi, m4v
          </p>
        </div>
      </div>
